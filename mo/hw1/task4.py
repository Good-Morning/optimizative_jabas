...
def create_logistic_q(X, y, lmd, w):
    w_range = range(len(w))
...
def create_f(X, y, lmd, w, h):
    ...
    return create_logistic_q(X, y, lmd, arg)
...
def straignh(f):
    ...
    if f(x_prev + 0.0 * delta) <= f(x_prev + 1.0 * delta):
        h = -delta
        x_cur = x_prev - delta
    else:
        h = delta
...
def logistic_gradient(X, y, lmd, w_begin, eps, max_iter_num):
    ...
    while True:
        f = partial(create_f, X, y, lmd, w_cur)
        a0, b0 = straignh(f)
        ...
        if abs(create_logistic_q(X, y, lmd, w_next) - create_logistic_q(X, y, lmd, w_cur)) < eps or iter_num == max_iter_num:
            return w_next
